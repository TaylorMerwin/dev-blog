...
async function createBlogPost(title, postDescription, content, authorId, imagePath) {
  ...
  VALUES ($1, $2, $3, $4, $5)
  RETURNING post_id`;
    const result = await database_1.pool.query(query, [title, postDescription, content, authorId, imagePath]);
...
async function updateBlogPost(postId, title, postDescription, content) {
  ...
  RETURNING post_id`;
    try {
        const result = await database_1.pool.query(query, [postId, title, postDescription, content]);
