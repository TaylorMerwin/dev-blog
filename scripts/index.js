...
/**
 * Loads the next batch of posts from the posts array and renders them.
 */
function loadPosts() {
  ...
  postsToLoad.forEach((post) => {
    ...
    image.src = post.images.length > 0 ? post.images[0] :
    ...
    cardTitle.innerHTML =
        `<h1><a href="view-blog.html?id=${post.id}">${post.title}</a></h1>`;
